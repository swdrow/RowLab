  ...
  return (
    <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-6">
      ...
      <div className="bg-ink-raised rounded-lg p-4 border border-ink-border">
        <h3 className="text-xs font-semibold text-txt-tertiary uppercase tracking-wider mb-2">
          Race Details
        </h3>
        <div className="text-sm text-txt-primary">
          ...
          {' • '}
          <span className="font-mono tabular-nums">{race.distanceMeters}m</span>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="space-y-3 divide-y divide-ink-border">
          {fields.map((field, index) => {
            const isOwnTeam = watch(`results.${index}.isOwnTeam`);
            return (
              <div
                key={field.id}
                className={`p-4 pt-3 first:pt-4 ${isOwnTeam ? 'bg-accent-copper/5' : ''}`}
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-shrink-0 w-8 h-8 rounded-full bg-ink-well flex items-center justify-center">
                    ...
                    {index > 2 && (
                      <span className="text-sm font-medium text-txt-secondary">{index + 1}</span>
                    )}
                  </div>
                  ...
                  <div className="flex-1 grid grid-cols-1 md:grid-cols-4 gap-3">
                    ...
                    <div>
                      <input
                        ...
                        min={1}
                        className="w-full px-3 py-2 bg-ink-well border border-ink-border rounded-lg
                               text-txt-primary placeholder:text-txt-tertiary text-sm font-mono tabular-nums
                               focus:outline-none focus:ring-2 focus:ring-focus-ring"
                      />
                      {errors.results?.[index]?.place && (
                        <p className="mt-1 text-xs text-data-poor">
                          {errors.results[index]?.place?.message}
                        </p>
                      )}
                    </div>
                    ...
                    <div>
                      <input
                        ...
                        className="w-full px-3 py-2 bg-ink-well border border-ink-border rounded-lg
                               ...
                               focus:outline-none focus:ring-2 focus:ring-focus-ring"
                      />
                      {errors.results?.[index]?.finishTimeStr && (
                        <p className="mt-1 text-xs text-data-poor">
                          {errors.results[index]?.finishTimeStr?.message}
                        </p>
                      )}
                    </div>
                  </div>
