...
import {
  NCAA_WEEKLY_LIMIT,
  NCAA_DAILY_LIMIT,
  NCAA_COMPETITION_HOURS,
} from '../../../utils/ncaaRules';
...
export function NCAAAuditReport({
  ...
  const handlePrint = () => {
    if (printRef.current) {
      ...
      if (printWindow) {
        // Print window uses inline styles (can't reference CSS variables in new window)
  ...
  if (error || !report) {
    return (
      <div className="text-center py-8 text-txt-tertiary">Failed to generate compliance report</div>
    );
  }
  ...
  return (
    <div className={`ncaa-audit-report ${className}`}>
      ...
      {/* Printable Content - uses V3 tokens for on-screen, print window has inline styles */}
      <div
        ref={printRef}
        className="bg-bg-surface text-txt-primary p-6 rounded-lg border border-bdr-default print:border-0 print:bg-white print:text-black"
      >
        {/* Header */}
        <div className="header mb-6">
          <h1 className="text-xl font-bold">NCAA 20-Hour Rule Compliance Report</h1>
          <p className="text-txt-secondary">{teamName}</p>
        ...
        {/* Summary */}
        <div className="summary bg-bg-surface-elevated p-4 rounded-lg mb-6">
          <h2 className="font-semibold mb-2">Summary</h2>
          <div className="grid grid-cols-3 gap-4 text-sm">
            <div>
              <div className="text-txt-secondary">Total Athletes</div>
              <div className="text-lg font-semibold font-mono">{report.entries.length}</div>
            </div>
            <div>
              <div className="text-txt-secondary">Near Limit (18-20h)</div>
              <div
                className={`text-lg font-semibold font-mono ${report.athletesNearLimit > 0 ? 'text-data-warning' : ''}`}
              >
                {report.athletesNearLimit}
              </div>
            </div>
            <div>
              <div className="text-txt-secondary">Over Limit (&gt;20h)</div>
              <div
                className={`text-lg font-semibold font-mono ${report.athletesOverLimit > 0 ? 'text-data-poor' : ''}`}
              >
        ...
        <div className="text-xs text-txt-tertiary mb-6 p-3 bg-bg-surface-elevated rounded">
          <strong>NCAA Rules Reference:</strong>
          <ul className="mt-1 space-y-0.5">
            <li>
              Weekly maximum: {NCAA_WEEKLY_LIMIT} hours of CARA (Countable Athletically Related
              Activities)
            </li>
            <li>Daily maximum: {NCAA_DAILY_LIMIT} hours per day</li>
            <li>
              Competitions: Count as {NCAA_COMPETITION_HOURS} hours regardless of actual duration
            </li>
          </ul>
        </div>
        ...
        {report.entries.map((entry) => (
          <div key={entry.athleteId} className="section mb-6">
            <h2 className="text-base font-semibold border-b-2 border-bdr-strong pb-1 mb-3 flex items-center justify-between">
            ...
            {entry.sessions.length === 0 ? (
              <p className="text-sm text-txt-tertiary italic">No countable activities recorded</p>
            ) : (
                  ...
                  {entry.sessions
                    ...
                    .map((session, idx) => (
                      <tr key={idx}>
                        ...
                        <td className="text-txt-tertiary">{session.notes || '-'}</td>
                      </tr>
                    ))}
                </tbody>
                <tfoot>
                  <tr className="font-semibold">
                    ...
                    <td className={`font-mono ${entry.isOverLimit ? 'text-data-poor' : ''}`}>
        ...
        {/* Footer */}
        <div className="footer mt-8 pt-4 border-t border-bdr-default text-xs text-txt-tertiary">
          <p>
            Report generated: {format(parseISO(report.generatedAt), "MMMM d, yyyy 'at' h:mm a")}
          </p>
          <p className="mt-1">
            This report is for internal compliance tracking purposes. Retain for NCAA audit
            requirements.
          </p>
        </div>
      </div>
