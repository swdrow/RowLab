  ...
  if (isLoading) {
    return (
      <div className="h-full flex flex-col">
        <div className="mb-6">
          <div className="h-8 w-48 bg-ink-base rounded animate-pulse mb-2" />
          <div className="h-4 w-32 bg-ink-base rounded animate-pulse" />
        </div>
        <div className="grid grid-cols-2 gap-4 flex-1">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="bg-ink-base rounded-lg animate-pulse" />
          ))}
        </div>
      </div>
    );
      ...
      <div className="h-full flex items-center gap-4">
        ...
        {personalStats.latestErgTime && (
          <div className="flex-1">
            ...
            <div className="text-2xl font-mono font-bold text-ink-bright">
              {personalStats.latestErgTime}
            </div>
          </div>
        )}
        ...
        {personalStats.attendanceStreak > 0 && (
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs text-ink-muted uppercase tracking-wide">Streak</span>
            </div>
            <div className="text-2xl font-mono font-bold text-ink-bright">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-ink-muted uppercase tracking-wide">PRs</span>
          </div>
          <div className="text-2xl font-mono font-bold text-ink-bright">
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-ink-muted">Here's your progress at a glance</p>
      </div>
      ...
      <div className={`grid gap-4 flex-1 ${size === 'expanded' ? 'grid-cols-3' : 'grid-cols-2'}`}>
        {/* Latest erg */}
        {personalStats.latestErgTime && (
          <div className="bg-ink-base border border-white/[0.06] rounded-lg p-4 flex flex-col">
            <div className="flex items-center gap-2 mb-2">
              <Target className="w-5 h-5 text-accent-copper" />
              <span className="text-xs text-ink-muted uppercase tracking-wide">
                {personalStats.latestErgTestType}
              </span>
              <PRBadge isPR={latestErgIsPR} />
            ...
            <div
              className={`text-3xl font-mono font-bold flex-1 flex items-center ${
                latestErgIsPR ? 'text-amber-400' : 'text-ink-bright'
              }`}
            >
              {personalStats.latestErgTime}
            </div>
            <p className="text-xs text-ink-muted mt-2">Latest test</p>
          </div>
        )}
        ...
        {personalStats.attendanceStreak > 0 && (
          <div className="bg-ink-base border border-white/[0.06] rounded-lg p-4 flex flex-col">
            ...
            <div className="text-3xl font-mono font-bold text-ink-bright flex-1 flex items-center">
              {personalStats.attendanceStreak}
            </div>
        ...
        {personalStats.overallRanking && (
          <div className="bg-ink-base border border-white/[0.06] rounded-lg p-4 flex flex-col">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs text-ink-muted uppercase tracking-wide">Ranking</span>
            </div>
            <div className="text-3xl font-mono font-bold text-ink-bright flex-1 flex items-center">
              #{personalStats.overallRanking}
            </div>
      ...
      {size === 'expanded' && (
        <div className="mt-4 p-4 bg-ink-base border border-white/[0.06] rounded-lg">
          <p className="text-xs text-ink-muted">Erg history sparkline coming soon</p>
        </div>
      )}
    </div>
  );
