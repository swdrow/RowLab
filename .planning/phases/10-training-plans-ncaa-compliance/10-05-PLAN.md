...
must_haves:
  truths:
    - "Coach can drag a workout to reschedule it"
    - "Dragged event moves to new date with visual feedback"
    - "Failed reschedule rolls back to original position"
    - "Optimistic update shows immediate visual change"
  artifacts:
    - path: "src/v2/components/training/calendar/DragDropCalendar.tsx"
      provides: "Calendar with drag-drop rescheduling"
      min_lines: 120
  key_links:
    - from: "src/v2/components/training/calendar/DragDropCalendar.tsx"
      to: "react-big-calendar/lib/addons/dragAndDrop"
      via: "withDragAndDrop HOC"
      pattern: "withDragAndDrop"
    - from: "src/v2/components/training/calendar/DragDropCalendar.tsx"
      to: "src/v2/hooks/useWorkouts.ts"
      via: "useRescheduleWorkout mutation"
      pattern: "useRescheduleWorkout"
---

<objective>
Add drag-drop rescheduling to the training calendar using react-big-calendar's drag-drop addon.

Purpose: Enable coaches to reschedule workouts by dragging them to new dates (TRAIN-03).
Output: DragDropCalendar component with optimistic updates and rollback on error.
</objective>
...
@.planning/phases/10-training-plans-ncaa-compliance/10-03-SUMMARY.md
@src/v2/components/training/calendar/TrainingCalendar.tsx
@src/v2/hooks/useWorkouts.ts (useRescheduleWorkout)
</context>

<tasks>

<task type="auto">
  <name>Task 1: Create DragDropCalendar component</name>
  <files>src/v2/components/training/calendar/DragDropCalendar.tsx</files>
  <action>
Create an enhanced calendar component with drag-drop functionality:
...
export function DragDropCalendar({
  ...
  const handleEventDrop = useCallback(
    ({ event, start, end, isAllDay }: withDragAndDropProps<CalendarEvent>['onEventDrop'] extends ((args: infer A) => any) ? A : never) => {
      ...
      // Trigger the reschedule mutation (optimistic update handled by hook)
      rescheduleWorkout(
        { id: workoutId, scheduledDate: newDate },
        {
          onSuccess: () => {
...
```
  ...
  <verify>
1. File exists at src/v2/components/training/calendar/DragDropCalendar.tsx
2. TypeScript compiles without errors: `npx tsc --noEmit src/v2/components/training/calendar/DragDropCalendar.tsx`
3. Imports react-big-calendar drag-drop addon correctly
  </verify>
  <done>
DragDropCalendar supports drag-to-reschedule with optimistic updates, dragging indicator, and proper styling.
  </done>
</task>

<task type="auto">
  <name>Task 2: Update calendar index exports</name>
  <files>src/v2/components/training/calendar/index.ts</files>
  <action>
Update index file to export the new DragDropCalendar component:
...
```
  ...
  <verify>
File exports DragDropCalendar alongside existing exports.
  </verify>
  <done>
Index exports TrainingCalendar, DragDropCalendar, CalendarToolbar, and WorkoutEventCard.
  </done>
</task>
...
<verification>
1. DragDropCalendar renders without errors
2. Events can be dragged to new dates
3. Drag preview shows semi-transparent event
4. Drop target highlights when hovering
5. Optimistic update moves event immediately
6. Failed API call rolls back event position
7. Recurring events cannot be dragged (intentional limitation)
</verification>

<success_criteria>
- Coach can drag workouts to reschedule (TRAIN-03)
- Visual feedback shows drag preview and drop target
- Optimistic update provides immediate feedback
- Error handling rolls back on API failure
- Non-recurring events are draggable
- Recurring event instances show appropriate behavior
</success_criteria>

<output>
After completion, create `.planning/phases/10-training-plans-ncaa-compliance/10-05-SUMMARY.md`
</output>
